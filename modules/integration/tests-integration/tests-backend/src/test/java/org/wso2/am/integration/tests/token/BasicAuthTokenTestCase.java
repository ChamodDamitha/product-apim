...
import org.wso2.am.integration.test.utils.clients.APIPublisherRestClient2;
 ...
 */
public class BasicAuthTokenTestCase extends APIMIntegrationBaseTest {

    private static final Log log = LogFactory.getLog(InvalidTokenTestCase.class);

    private APIPublisherRestClient2 apiPublisher;
    ...
    public BasicAuthTokenTestCase(TestUserMode userMode) {
    ...
    public void setEnvironment() throws Exception {
        ...
        apiPublisher = new APIPublisherRestClient2("https://localhost:9943/");
        provider = user.getUserName();
    }

    @Test(groups = "wso2.am", description = "Check functionality of API access with invalid basic auth token")
    public void testAPIAccessWithInvalidToken() throws XPathExpressionException, APIManagerIntegrationTestException {
        ...
        APIRequestRestAPI apiRequest = null;
        try {
            apiRequest = new APIRequestRestAPI(API_NAME, apiContext, new URL(endpointUrl));
        ...
        apiRequest.addOperation(new Operation("/", "GET", "Unlimited", "Application & Application User"));
        apiRequest.addOperation(new Operation("/", "POST", "Unlimited", "Application & Application User"));
        apiRequest.setSecurityScheme(new String[]{"basic_auth"});

        HttpResponse apiResponse = apiPublisher.addAPI(apiRequest);
        JSONObject apiJsonResponse = null;
        try {
            apiJsonResponse = new JSONObject(apiResponse.getData());
        ...
        //publishing API
        try {
            apiPublisher.changeAPILifeCycleStatus(apiJsonResponse.getString("id"), "Publish");
        } catch (JSONException e) {
            log.error(e);
        }
        ...
        headers.put("Authorization", "Basic " + "abcdefgh");
